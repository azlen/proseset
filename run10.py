...
- Longer made words are worth more
- Novel/rare made words are worth more (diminishing returns for common ones)
- Reports longest and most common made words at the end
...
class Template:
    ...
    interior_segments: Tuple[str, ...]  # Segments that don't merge (appear as-is in output)
    interior_length: int  # Total length of interior segments
...
class DeckState:
    ...
    # Track how often each interior segment appears in enabled templates
    interior_segment_count: Dict[str, int] = field(default_factory=dict)
...
def build_lookups(deck_words: Set[str], seg_words: Set[str]) -> Lookups:
    ...
    for word in progress(deck_words, "Building templates"):
        ...
        for seg in segment_word(word, require_start=False, require_end=False):
            ...
            # Interior segments: everything except first and last (they merge)
            interior = seg[1:-1] if len(seg) > 2 else ()
            interior_len = sum(len(s) for s in interior)

            # Score: product of segment frequencies * word frequency
            # Bonus for longer interior segments (more interesting words created)
            score = get_zipf(word)
            for s in seg:
                score *= score_segment(s)
            # Length bonus: sqrt of interior length
            score *= (1 + interior_len ** 0.5 * 0.2)
...
def add_word_to_state(word: str, state: DeckState, lookups: Lookups):
    ...
    # Track interior segments from this word's templates
    for t in lookups.templates_as_middle.get(word, []):
        for seg in t.interior_segments:
            state.interior_segment_count[seg] = state.interior_segment_count.get(seg, 0) + 1
...
def remove_word_from_state(word: str, state: DeckState, lookups: Lookups):
    ...
    # Remove interior segments from this word's templates
    for t in lookups.templates_as_middle.get(word, []):
        for seg in t.interior_segments:
            state.interior_segment_count[seg] = state.interior_segment_count.get(seg, 0) - 1
...
def compute_marginal_value(candidate: str, state: DeckState, lookups: Lookups) -> Tuple[float, int, Tuple[int, int, int]]:
    ...
    for t in lookups.templates_as_middle.get(candidate, []):
        ...
        if left > 0 and right > 0:
            ...
            # Apply novelty penalty: interior segments that already appear often are less valuable
            novelty_mult = 1.0
            for seg in t.interior_segments:
                existing_count = state.interior_segment_count.get(seg, 0)
                # Diminishing returns: 1/(1+count) so first occurrence is full value
                novelty_mult *= 1.0 / (1.0 + existing_count * 0.5)

            middle_combos += combos
            total_score += t.score * combos * novelty_mult
...
def analyze_deck(deck: List[str], lookups: Lookups):
    ...
    # Collect all made words (segments from all triplets)
    print(f"\n{'='*60}")
    print("MADE WORDS ANALYSIS")
    print(f"{'='*60}")

    made_word_counts: Dict[str, int] = {}
    ...
    for mid in deck:
        for t in lookups.templates_as_middle.get(mid, []):
            ...
            if left_count > 0 and right_count > 0:
                triplet_count = left_count * right_count
                # Interior segments appear in every triplet using this template
                for seg in t.interior_segments:
                    made_word_counts[seg] = made_word_counts.get(seg, 0) + triplet_count
                # The merged segments depend on specific left/right words, but
                # we can approximate by counting the first_key and last_key merges
                # (they form parts of words but are harder to enumerate exactly)

    # Also count the interior segments from templates where word is left or right
    # But for simplicity, let's focus on interior segments from middle templates

    if made_word_counts:
        # Sort by count (most common)
        by_count = sorted(made_word_counts.items(), key=lambda x: -x[1])
        print(f"\nMost commonly made words (interior segments):")
        for word, count in by_count[:20]:
            print(f"  {word:<15} appears in {count:6d} triplets")

        # Sort by length (longest)
        by_length = sorted(made_word_counts.items(), key=lambda x: (-len(x[0]), -x[1]))
        print(f"\nLongest made words:")
        for word, count in by_length[:20]:
            print(f"  {word:<15} (len={len(word)}) appears in {count:6d} triplets")

        # Unique made words
        print(f"\nTotal unique interior segments: {len(made_word_counts)}")
    else:
        print("\nNo interior segments found (all templates have only 2 segments)")
