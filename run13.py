...
BANNED_WORDS = {
    ...
    'qat', 'qis', 'qua',
    'rai', 'raj', 'ras', 'rax', 'reb', 'rec', 'ree', 'rei', 'ret', 'ria', 'rif', 'rin', 'rom', 'rya',
...
def update_made_words_for_triplet(left: str, mid: str, right: str, state: DeckState, delta: int):
    ...
    if segmentations:  # Only count as connection if it produces valid segmentations
        ...
        # Original triplet for comparison
        original_seg = (left, mid, right)

        for seg in segmentations:
            # Only count if boundaries differ from the original triplet
            # The alternate can be any number of words (1, 2, 3+) as long as boundaries changed
            if seg == original_seg:
                continue
            for made_word in seg:
                # Find which source words actually contribute to this made-word
                contributors = find_contributing_words(made_word, concat, left, mid, right)

                # Only track as unique if all contributing words are different
                # e.g., "vermillion" from "over + million + X" should only count once regardless of X
                if made_word not in state.made_word_recipes:
                    state.made_word_recipes[made_word] = set()

                if delta > 0:
                    state.made_word_recipes[made_word].add(contributors)
                else:
                    state.made_word_recipes[made_word].discard(contributors)
...
def compute_word_contribution(word: str, state: DeckState, lookups: Lookups) -> Tuple[float, int]:
    ...
    if word in state.word_contributions:
        for made_word, count in state.word_contributions[word].items():
            ...
            # Rarity bonus based on how rare this made-word is (# of unique recipes)
...
def enumerate_word_triplets(word: str, state: DeckState, lookups: Lookups) -> Dict[str, int]:
    ...
    for t in lookups.templates_as_middle.get(word, []):
        for left_word in deck_list:
            ...
            for right_word in deck_list:
                ...
                if segmentations:
                    original_seg = (left_word, word, right_word)
                    for seg in segmentations:
                        # Only count if boundaries differ from the original triplet
                        if seg == original_seg:
                            continue
    ...
    for t in lookups.templates_as_left.get(word, []):
        ...
        for right_word in deck_list:
            ...
            if segmentations:
                original_seg = (word, t.middle_word, right_word)
                for seg in segmentations:
                    # Only count if boundaries differ from the original triplet
                    if seg == original_seg:
                        continue
    ...
    for t in lookups.templates_as_right.get(word, []):
        ...
        for left_word in deck_list:
            ...
            if segmentations:
                original_seg = (left_word, t.middle_word, word)
                for seg in segmentations:
                    # Only count if boundaries differ from the original triplet
                    if seg == original_seg:
                        continue
...
def analyze_deck(deck: List[str], lookups: Lookups, state: DeckState = None):
    ...
    if made_word_recipes:
        ...
        print(f"\nLongest made words:")
        for i, (word, recipes) in enumerate(by_length[:30]):
            num_recipes = len(recipes)
            print(f"  {word:<20} (len={len(word):2d}) created by {num_recipes:3d} unique word combination{'s' if num_recipes != 1 else ''}")
            # Show example recipes for first few
            if i < 5:
                for j, recipe in enumerate(list(recipes)[:3]):
                    print(f"      e.g., {' + '.join(recipe)}")
                if num_recipes > 3:
                    print(f"      ... and {num_recipes - 3} more")

        # Sort by number of unique ways to create (diversity)
        by_diversity = sorted(made_word_recipes.items(), key=lambda x: -len(x[1]))
        print(f"\nMost versatile made words (most unique ways to create):")
        for i, (word, recipes) in enumerate(by_diversity[:30]):
            num_recipes = len(recipes)
            print(f"  {word:<20} created by {num_recipes:3d} unique word combination{'s' if num_recipes != 1 else ''}")
            # Show example recipes for first few
            if i < 5:
                for j, recipe in enumerate(list(recipes)[:3]):
                    print(f"      e.g., {' + '.join(recipe)}")
...
def main():
    ...
    deck, state = build_deck_greedy(seeds, 30, deck_words, lookups)
