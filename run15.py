...
- Base score = left_percent * right_percent (naturally optimizes for balance)
- Refined scoring (made-word bonus) only on top 10 candidates
"""
...
def compute_word_contribution(word: str, state: DeckState, lookups: Lookups) -> Tuple[float, int]:
    ...
    connectivity_score = left_percent * right_percent * 10
...
def compute_marginal_value(candidate: str, state: DeckState, lookups: Lookups) -> Tuple[float, int, Tuple[int, int, int]]:
    ...
    # Base score: product of percentages (naturally rewards balance)
    # Scale by 10 to get reasonable numbers (max would be 100*100*10 = 100,000)
    connectivity_score = left_percent * right_percent * 10
...
def main():
    ...
    deck, state = build_deck_greedy(seeds, 150, deck_words, lookups)
