...
- e.g., if first_key="edge", scores higher if "knowledge" (9 letters) is possible vs "ledge" (5 letters)
- Optimizes for potential to create longer spanning words
...
def build_deck_greedy(
    ...
    while len(deck_list) < target_size or (len(deck_list) >= target_size and refinement_iterations < max_refinement):
        ...
        if len(deck_list) < target_size:
            print(f"{len(deck_list):3d}. {GREEN}+{best_word:<12}{RESET} ~{best_combos:4d} combos (L={l:3d} M={m:3d} R={r:3d}) freq={freq:.1f}")
        else:
            ...
            print(f"  Swap {refinement_iterations:3d}: {GREEN}+{best_word:<12}{RESET} (L={l:3d} M={m:3d} R={r:3d}) freq={freq:.1f}")
        ...
        if should_prune:
            ...
            if worst_word:
                # Check for cycling in refinement phase
                if len(deck_list) >= target_size and worst_word == last_removed:
                    print(f"     Refinement complete: cycling detected (would re-remove {worst_word})")
                    break
                ...
                if len(deck_list) < target_size:
                    print(f"     {RED}-{worst_word:<12}{RESET} (contributed {worst_combos} combos, score={worst_score:.1f})")
                else:
                    print(f"                  {RED}-{worst_word:<12}{RESET}")
...
def analyze_deck(deck: List[str], lookups: Lookups):
    ...
    # Enumerate ALL triplets and calculate all made words
    print(f"\n{'='*60}")
    print("MADE WORDS ANALYSIS (from all triplets)")
    print(f"{'='*60}")
    ...
    print(f"\nTotal valid triplets: {total_triplets}")
...
def main():
    ...
    deck = build_deck_greedy(seeds, 200, deck_words, lookups)
